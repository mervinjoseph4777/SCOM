...
<body>
			...
			<div id="sidebar" class="nav-collapse">
				...
				<div class="leftside-navigation">
					<ul class="sidebar-menu" id="nav-accordion">
						...
						<li>
							<a class="active"  href="newsletters.php">
                    ...
                    <div class="panel panel-default">
                        ...
                        <div class="row w3-res-tb">
                            <div class="col-sm-5 m-b-xs">
                                <button class="btn btn-sm btn-info" style="padding:15px 30px;margin:1rem;" onclick="copyData()">Copy Data</button>
                            </div>
                            <div class="col-sm-4">
                            </div>
    ...
    <script>
        $(document).ready(function () {
            ...
            getNewsLettersInfo(infoStatus);
        });

        function getNewsLettersInfo(infoStatus){
            var xmlhttp = new XMLHttpRequest();
            xmlhttp.open("POST", "newsletters-info.php?status="+infoStatus, true);
            xmlhttp.send();
        ...
        function changeStatus(){
            ...
            getNewsLettersInfo(output);
        }

        
        function copyData() {
            var copyText = document.getElementsByClassName("emails");
            const emails = [];
            for(var i=0;i<copyText.length;i++){     
                emails[i] = copyText[i].innerHTML
            }
            console.log(emails);
        ...
        function deleteNewsLetters(key){
            // console.log(key);
            let text = "Are you sure you want to delete?";
            if (confirm(text) == true) {
                var xmlhttp = new XMLHttpRequest();
                xmlhttp.open("POST", "newsletters-delete.php?id="+key, true);
                xmlhttp.send();
                xmlhttp.onreadystatechange = function () {
                    if (this.readyState == 4 && this.status == 200) {
                        location.reload();
